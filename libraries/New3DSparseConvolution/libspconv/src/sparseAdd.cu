...
static __global__ void add_kernel(size_t num_indices, Tin* input_1,
...
template <>
__global__ void add_kernel<int8_t, int8_t>(size_t num_indices, int8_t* input_1,
                                           float input_scale_1, int8_t* input_2,
                                           float input_scale_2, int8_t* output,
                                           float output_scale) {
  int idx = cuda_linear_index;
  if (idx >= num_indices) return;
  // add
...
template <>
__global__ void add_kernel<half, int8_t>(size_t num_indices, half* input_1,
                                         float input_scale_1, half* input_2,
                                         float input_scale_2, int8_t* output,
                                         float output_scale) {
  int idx = cuda_linear_index;
  if (idx >= num_indices) return;
...
SparseAdd::SparseAdd(std::string input_name_0, std::string input_name_1,
...
void SparseAdd::forward(
  ...
  if (int8_inference_) {
    if (input_precision_ == "int8" && output_precision_ == "int8") {
      cuda_linear_launch(
          add_kernel, _stream, num_indices,
  ...
  io_dict[output_name_] = std::make_shared<spconv::DTensorImplement>(
      input1->features_shape(), input1->features_dtype(), (void*)output_,
