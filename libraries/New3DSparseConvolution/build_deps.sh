...
SPCONV_LIB=$LIBSPCONV_DIR/build/spconv/src/libspconv.so
...
if ! [[ -f $SPCONV_LIB ]]; then
  if ! [[ -d $SPCONV_DIR ]]; then
    git clone --branch v2.3.6 https://github.com/traveller59/spconv.git $SPCONV_DIR
  else
    echo "SPCONV exist at $SPCONV_DIR"
  fi 

  if ! [[ -d $LIBSPCONV_DIR/cumm ]]; then
    git clone --branch v0.4.11 https://github.com/FindDefinition/cumm.git $LIBSPCONV_DIR/cumm
  ...
  cd $SPCONV_DIR
  echo "__version__ = '2.3.6'" > spconv/__version__.py # hack so as not to install spconv
